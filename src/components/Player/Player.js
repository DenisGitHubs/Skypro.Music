...
function handleBack () {
  let index = playListIsPlaying.findIndex(el => el.id === songIsPlaying.id)
  index = index - 1
  const searchTrack = playListIsPlaying[index];
  if (!searchTrack) {
    const  handleSong = playListIsPlaying[playListIsPlaying.length -1]
    dispatch(setSongIsPlaying({handleSong}))
  } else {
    const handleSong = playListIsPlaying[index]
    dispatch(setSongIsPlaying({handleSong}))
  }
  const dataTogglePlayer = true
  dispatch(togglePlayer({dataTogglePlayer}))
}
...
function handleNext() {
  ...
  const searchTrack = playListIsPlaying[index];
    if (!searchTrack) {
    const handleSong = playListIsPlaying[0]
    dispatch(setSongIsPlaying({handleSong}))
    } else {
      const handleSong = playListIsPlaying[index]
      dispatch(setSongIsPlaying({handleSong}))
...
if (isLoop) {
  const data = false
  dispatch(toggleShaffle({data}))
} else {
  const data = true
  dispatch(toggleShaffle({data}))
}
}
  const togglePlay = playMusic ? handleStop : handleStart;
  ...
  const togglePlayIcon = playMusic ? <svg xmlns="http://www.w3.org/2000/svg" width="15" height="19" viewBox="0 0 15 19" fill="none">
...
<path d="M10 15L15 17.8868V12.1132L10 15ZM13 3.5C16.0376 3.5 18.5 5.96243 18.5 9H19.5C19.5 5.41015 16.5899 2.5 13 2.5V3.5ZM18.5 9C18.5 12.0376 16.0376 14.5 13 14.5V15.5C16.5899 15.5 19.5 12.5899 19.5 9H18.5Z" fill="white"/>
    ...
    function TotalTimeSec() {
        let time = audioRef.current.duration % 60;
        if(time < 10) {
    ...
    function shuffleTracks() {
      if (isShuffle) {
        ...
        const newList = []
        newList.push(playListIsPlaying.map((item) => (
            {
              id: item.id,
              name: item.name,
              author: item.author,
...
const handleDisLike = async () => {
  ...
  return (
    <>
    <audio onEnded={() => handleNext()} onPlaying={() => {LoadData(); CurrentTime(audioRef.current.currentTime); progressBarTime()}} autoPlay ref={audioRef} src={songIsPlaying.track_file}>
      ...
      <S.BarPlayerBlock>
        <S.BarPlayer>
          <S.PlayerControls>
            ...
            <div className="player__btn-shuffle _btn-icon" onClick={() => shuffleTracks()}>
          ...
          <S.PlayerTrackPlay>
            <S.TrackPlayContain>
              ...
              <S.TrackPlayAuthor>
                <a className={!loadingFromApi ? "track-play__author-link" : "track-play__author-link skeleton-block skeleton-effect-fade"} href="http://">
                  {loadingFromApi ? text : `${songIsPlaying.name}`}
                </a>
              </S.TrackPlayAuthor>
              <S.TrackPlayAlbum>
                <a className={!loadingFromApi ? "track-play__album-link" : "track-play__album-link skeleton-block skeleton-effect-fade"} href="http://">
                {loadingFromApi ? text : `${songIsPlaying.author}`}
                </a>
              </S.TrackPlayAlbum>
            </S.TrackPlayContain>
            <S.TrackPlayLikeDis>
